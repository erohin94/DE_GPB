...
INSERT INTO sandbox_hr.course_review_filtered_responses_tmp
...
FROM dm_source_hdp.hr_edu_response_source 
WHERE 1 = 1
    AND object_id IS NOT NULL
    AND `comment` IS NOT NULL
    AND LENGTH(REGEXP_REPLACE(`comment`, '.', 'X')) > 6
/
COMPUTE STATS sandbox_hr.course_review_filtered_responses_tmp						    
/


INSERT INTO sandbox_hr.course_review_recent_responses_tmp
...
SELECT 
      ...
      CASE /*Задаем флаг новизны отзыва*/
       ...
       ELSE 0
      END AS include_flag
FROM sandbox_hr.course_review_filtered_responses_tmp
WHERE total_responses >= 10
/
COMPUTE STATS sandbox_hr.course_review_recent_responses_tmp
/


INSERT INTO sandbox_hr.course_review_final_responses_tmp
...
FROM sandbox_hr.course_review_recent_responses_tmp
/
COMPUTE STATS sandbox_hr.course_review_final_responses_tmp
/


INSERT INTO sandbox_hr.ees_course_review_input
/*Итоговая таблица*/
...
FROM dm_source_hdp.hr_edu_catalog_source c 
JOIN sandbox_hr.course_review_final_responses_tmp r ON c.object_id = r.object_id
WHERE r.final_include_flag = 1
    AND r.rn_2 <= 500
    AND r.total_meaningful_review >= 10
/
COMPUTE STATS sandbox_hr.ees_course_review_input
/
