...
В CLOUDERA manager получили следующую инфу по запросу, при создании данной таблицы.
...
Когда делаю джоин этих таблиц, то в `WHERE` я указываю фильтр по годам. Движок отбрасывает лишние партиции (PRUN - обрезать). Читает только те файлы где есть нужные данные.
